...
<script type="text/javascript" charset="utf-8" src="js/lang/zh_CN.js"></script>
		...
		%> 
			<form id="articleModifyForm" action="doUpdate.asp" onsubmit="return toVaild()" method="post">
            <%
				dim id
			...
			%>
                ...
                <li>
                 文章编号：<input style="height:24px; font-size:16px" type="text" name="id" id="id" value=" <%=rs("id")%>"  readonly="readonly" />
					...
					<ul>
						<li >文章标题:<input style="height:24px; font-size:16px" type="text" name="title" id="title" value=" <%=rs("title")%>"/></li>	
                        </ul>
                        </div>
                    <div class="operator-text">
                        <ul>
